...
def specmatch_spectrum(args):
    ...
    if args.debug:
        ...
        lib_subset = np.random.choice(lib_subset, size=args.n_lib_subset,
                                      replace=False)
        lib_subset = np.sort(lib_subset)
        lib_subset = list(lib_subset)

    core.specmatch_spectrum(args.spectrum, indir=args.directory,
                            plot_level=args.plots, inlib=args.in_library,
                            outdir=args.outdir, num_best=args.num_best,
...
def main():
    ...
    psr_sm.add_argument("-d", "--directory", type=str,
    ...
    psr_sm.add_argument("--n_lib_subset", type=int, default=NSPEC_DEBUG,
                        help="Number of random stars to select from library." +
                        " Useful for the purposes of debugging")
    psr_sm.add_argument("--debug", action='store_true',
